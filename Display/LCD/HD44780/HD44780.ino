/*
  LCD modules based on the HD44780 chip
  
  - VDD supply is 5V.
  
  - The HD44780 chip is controlled by 4-bit (D4 - D7) or 8-bit 
    (D0 - D7) I/O data lines and three additional signals:
    * With the register select (RS) pin we select between data registers 
      and command instructions.
    * A pulse on the Enable (E) pin triggers the writing to the registers.
    * The Read/Write (R/W) pin selects reading mode or writing mode.
      We tie it to GND because we only want to write to the display and 
      not read from it.

  - There is a display contrast pin (VO) that is controlled with a 
    voltage between 0V - 5V. 
    Important: wire that pin to see something on the display, use a 
               10kΩ potentiometer for that.
    
  - There are two LED backlight pins. The vast majority of LCDs already 
    include a 100Ω - 220Ω series resistor for the LED backlight, if it
    is missing, add it. Connect the anode to 5V and the cathode to GND.
...
const uint8_t EN_PIN = 11;
const uint8_t D4_PIN = 5;
const uint8_t D5_PIN = 4;
const uint8_t D6_PIN = 3;
const uint8_t D7_PIN = 2;
LiquidCrystal lcd(RS_PIN, EN_PIN, D4_PIN, D5_PIN, D6_PIN, D7_PIN); // four bit mode without R/W pin
...
void setup()
{
  ...
  {
    lcd.setCursor(0, 1);
    lcd.write((uint8_t)0); // the 0 must be cast to uint8_t
    lcd.write(1);
    lcd.write(2);
    lcd.write(3);
    lcd.write(4);
