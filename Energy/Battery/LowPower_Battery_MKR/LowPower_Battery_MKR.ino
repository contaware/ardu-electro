/*
  ...
  - idle():      only the CPU is stopped. This mode allows power 
                 optimization with the fastest wake-up time. Note that 
                 it wakes-up after 1ms because of the SysTick interrupt, 
                 thus idle(millis) makes not much sense:
                 https://github.com/arduino-libraries/ArduinoLowPower/issues/39
                 
  - sleep():     this mode allows power optimization with a slower 
                 wakeup time. Only chosen peripherals are on (on MKR Zero 
                 powered from battery consumption is 0.7mA).
                 
  - deepSleep(): this mode allows power optimization with the slowest 
                 wake-up time. All but the RTC peripherals are stopped. 
                 Only the RTC or interrupt capable pins can wakeup the 
                 CPU (on MKR Zero powered from battery consumption is 
                 0.7mA, I could not measure a difference with the 
                 sleep() mode).
