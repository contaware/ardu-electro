/*
  ...
  - VDD supply is 3.7V - 5.2V (~70mA) and communication signal levels 
    are 3.3V.

  - SDA and SCL require pull-ups to 3.3V. Use 10kΩ for 100kHz and 2kΩ 
    for 400kHz.
  
  - It measures distances between 0.2m - 8m and has an accuracy of 6cm 
    in the 0.2m - 3m range and 2% in the 3m - 8m range.
  ...
  - When the sensor faces up, we have from left to right:
    ...
    * Mode pin connected to GND: 
      I2C (default address is 0x10, see TFL_DEF_ADR)
    * Mode pin disconnected or connected to 3.3V: 
      UART (default baudrate is 115200)
    * Data signal can be setup as a data ready signal or it can be setup 
      as a on/off signal to detect objects closer than a configured 
      distance.
*/
#include <Wire.h>
